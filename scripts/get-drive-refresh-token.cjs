// scripts/get-drive-refresh-token.cjs
// Script to get OAuth refresh token for Google Drive API
const http = require("http");
const { google } = require("googleapis");
const { exec } = require("child_process");

const PORT = 5557; // Different port from other scripts

const CLIENT_ID = process.env.DRIVE_CLIENT_ID || process.env.GOOGLE_CLIENT_ID;
const CLIENT_SECRET = process.env.DRIVE_CLIENT_SECRET || process.env.GOOGLE_CLIENT_SECRET;

if (!CLIENT_ID || !CLIENT_SECRET) {
  console.error("Error: Missing OAuth credentials");
  console.error("Set DRIVE_CLIENT_ID and DRIVE_CLIENT_SECRET (or GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET) env vars first.");
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    console.log("\n" + "=".repeat(60));
    console.log("GOOGLE DRIVE OAUTH TOKENS RECEIVED");
